...
=============================
Trains 4 models (Ridge, RandomForest, XGBoost, LightGBM), tunes top models
with Optuna, and compares using RMSE/MAE/MAPE/R².
"""
...
def train_and_evaluate(features_df, feature_cols, target="clv",
                       output_dir="outputs", n_tune_trials=30):
    ...
    for name, model in models.items():
        joblib.dump(model, os.path.join(output_dir, "models", f"{name}.joblib"))
    joblib.dump(feature_cols, os.path.join(output_dir, "models", "feature_cols.joblib"))
    ...
    _save_fig(fig, "12_model_comparison", os.path.join(output_dir, "figures"))
    ...
    _save_fig(fig, "13_residuals", os.path.join(output_dir, "figures"))

    # Cross-validation on best model
    print(f"\n📊 5-Fold CV for {best_name}...")
