...
def main():
    ...
    run_eda(customers, transactions, features, os.path.join(output_dir, "figures"))

    # ── Step 4: Customer Segmentation ─────────────────────────────────────
    print("\n📦 STEP 4/7: Customer Segmentation")
    features = segment_customers(features, os.path.join(output_dir, "figures"))
    ...
    models, results_df, best_model, best_key, splits = train_and_evaluate(
        features, feature_cols, output_dir=output_dir, n_tune_trials=30
    )
    ...
    explain_model(best_model, X_train, X_test, feature_cols,
                  os.path.join(output_dir, "figures"), model_type="tree")
